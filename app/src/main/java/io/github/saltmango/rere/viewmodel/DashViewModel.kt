package io.github.saltmango.rere.viewmodel
...
import io.github.saltmango.rere.dash.DashKeepAliveService
import io.github.saltmango.rere.dash.DashSession
import io.github.saltmango.rere.dash.DashState
import io.github.saltmango.rere.dash.DashWifiManager
import io.github.saltmango.rere.dash.WifiConnStatus
import io.github.saltmango.rere.dash.map.LocationTracker
import io.github.saltmango.rere.dash.map.MapRenderer
import io.github.saltmango.rere.dash.map.Mercator
import io.github.saltmango.rere.dash.map.TileProvider
import io.github.saltmango.rere.dash.nav.GeoPoint
import io.github.saltmango.rere.dash.nav.Route
import io.github.saltmango.rere.dash.nav.Router
import io.github.saltmango.rere.dash.protocol.DashCommands
import io.github.saltmango.rere.dash.video.DashEncoder
import io.github.saltmango.rere.dash.video.NalProcessor
import io.github.saltmango.rere.dash.video.RtpPacketizer
...
class DashViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private val dashConfig  = io.github.saltmango.rere.dash.DashConfig.get(app)
    private val voice        = io.github.saltmango.rere.dash.nav.VoiceManager.get(app)
    private val repo         = io.github.saltmango.rere.data.SyncRepository.get(app)
    private val recorder     = io.github.saltmango.rere.data.RideRecorder()
    ...
    private val mediaInfo   = io.github.saltmango.rere.media.MediaInfoProvider(app)
    private val callController = io.github.saltmango.rere.media.CallController(app)
    ...
    companion object {
        ...
        // and 45 s wasn't enough to safely pull over + restart before reRe gave up. With the
    ...
    init {
        ...
        viewModelScope.launch {
            wifiManager.state.collect { ws ->
                io.github.saltmango.rere.data.RideDiagnostics.log(
                    "wifi",
                    ws.status.toString() +
                ...
                when (ws.status) {
                    ...
                    WifiConnStatus.ERROR -> {
                        ...
                        io.github.saltmango.rere.data.RideDiagnostics.stop("wifi error / dash gone")
                    }
                    else -> refreshStage()
                }
            }
        ...
        viewModelScope.launch {
            session.state.collect { state ->
                io.github.saltmango.rere.data.RideDiagnostics.log("session", "→ $state")
                refreshStage()
                ...
                when (state) {
                    ...
                    DashState.STREAMING -> {
                        ...
                        io.github.saltmango.rere.util.Telemetry.logEvent("dash_streaming")
                    }
        ...
        session.onError = { msg ->
            io.github.saltmango.rere.data.RideDiagnostics.log("error", msg)
        ...
        session.onButton = { btn ->
            ...
            val call = io.github.saltmango.rere.media.CallInfoProvider.incomingCall.value
            ...
            io.github.saltmango.rere.data.RideDiagnostics.log("button",
    ...
    fun connect() {
        ...
        io.github.saltmango.rere.data.RideDiagnostics.init(getApplication())
        io.github.saltmango.rere.data.RideDiagnostics.start("connect")
        ...
        io.github.saltmango.rere.data.RideDiagnostics.log(
            "build", "apk=${io.github.saltmango.rere.util.BuildId.sha12(getApplication())}" +
                " vc=${io.github.saltmango.rere.BuildConfig.VERSION_CODE} name=${io.github.saltmango.rere.BuildConfig.VERSION_NAME}",
        )
        io.github.saltmango.rere.data.RideDiagnostics.log(
    ...
    fun disconnect() {
        ...
        io.github.saltmango.rere.data.RideDiagnostics.log("connect", "user disconnect")
        stopRecording()        // a connect→disconnect session = one saved ride
        session.disconnect()
        wifiManager.disconnect()
        releaseBackgroundResources()
        refreshStage()
        io.github.saltmango.rere.data.RideDiagnostics.stop("disconnect")
        ...
        io.github.saltmango.rere.data.DiagnosticsUploader.uploadPending(getApplication())
    }
    ...
    private fun giveUp(reason: String) {
        ...
        releaseBackgroundResources()
        refreshStage()
        io.github.saltmango.rere.data.RideDiagnostics.log("error", "gave up: $reason — background released")
        io.github.saltmango.rere.data.RideDiagnostics.stop(reason)
    }
    ...
    private fun armConnLostAlert() {
        ...
        connLostAlertJob = viewModelScope.launch {
            ...
            if (session.state.value != DashState.STREAMING && userWantsConnection) {
                ...
                io.github.saltmango.rere.data.RideDiagnostics.log("connect", "sustained link loss → rider alert (vibrate + voice)")
                runCatching {
    ...
    fun exitNavigation() {
        ...
        session.updateRouteCard("reRe")   // dash card → name + 0.0 km, nav off
    }
    ...
    private fun startStream() {
        io.github.saltmango.rere.data.RideDiagnostics.log("stream", "startStream — encoder up, RTP→dash beginning")
        videoPtsMs = 0L
        ...
        val onEncoded: (ByteArray, Boolean) -> Unit = { annexB, isKey ->
            ...
            if (!loggedFirstFrame) {
                ...
                io.github.saltmango.rere.data.RideDiagnostics.log("stream", "first video frame sent (key=$isKey, ${annexB.size}B)")
            }
    ...
    private fun tick(): Boolean {
        ...
        val sig = buildString {
            ...
            append(io.github.saltmango.rere.media.CallInfoProvider.incomingCall.value?.takeIf { it.incoming }?.caller ?: "")
        }
    ...
    private fun maybeReroute(offRoute: Boolean, loc: android.location.Location?) {
        ...
        io.github.saltmango.rere.data.RideDiagnostics.log("reroute", "off-route ${offSec}s → requesting new route")
        ...
        viewModelScope.launch {
            ...
            if (r != null) {
                ...
                io.github.saltmango.rere.data.RideDiagnostics.log("reroute", "new route in ${took}ms (${r.geometry.size} pts, ${r.totalMeters.toInt()} m)")
            } else {
                android.util.Log.w("DashViewModel", "Reroute failed (no internet?)")
                io.github.saltmango.rere.data.RideDiagnostics.log("reroute", "FAILED after ${took}ms (no internet?)")
            }
            rerouting = false
        }
    }
    ...
    private fun answerCall(c: io.github.saltmango.rere.media.IncomingCall) {
        val sent = c.answerIntent?.let { runCatching { it.send() }.isSuccess } ?: false
        if (!sent) callController.answer()
        io.github.saltmango.rere.data.RideDiagnostics.log("media", "answer (notifAction=$sent)")
    }

    /** Reject (ringing) or hang up (active) via the dialer's action; fall back to TelecomManager. */
    private fun endCall(c: io.github.saltmango.rere.media.IncomingCall) {
        val sent = c.declineIntent?.let { runCatching { it.send() }.isSuccess } ?: false
        if (!sent) callController.hangup()
        io.github.saltmango.rere.data.RideDiagnostics.log("media", "end/reject (notifAction=$sent)")
    }
    ...
    private data class NavState(
        ...
        val nextManeuver: io.github.saltmango.rere.dash.nav.Maneuver?,
    )
    ...
    private fun trackProgress(r: Route, pos: GeoPoint): NavState {
        ...
        val nextMan = r.maneuvers.firstOrNull {
            it.cumulativeMeters > progressM + 1.0 && it.type != io.github.saltmango.rere.dash.nav.ManeuverType.DEPART
        }
    ...
    private fun startMediaForwarding() {
        val granted = io.github.saltmango.rere.media.MediaInfoProvider.isAccessGranted(getApplication())
        io.github.saltmango.rere.data.RideDiagnostics.log("media", "forwarding start — notification access=$granted")
        mediaInfo.start()
        mediaObserveJob?.cancel()
        mediaObserveJob = viewModelScope.launch {
            launch {
                mediaInfo.nowPlaying.collect { np ->
                    io.github.saltmango.rere.data.RideDiagnostics.log("media", "now-playing observed: ${np?.title ?: "(none)"}")
                    session.updateNowPlaying(np?.title, np?.album.orEmpty(), np?.artist.orEmpty())
                }
            }
            launch {
                io.github.saltmango.rere.media.CallInfoProvider.incomingCall.collect { call ->
                    io.github.saltmango.rere.data.RideDiagnostics.log("media", "call observed: ${call?.caller ?: "(none)"}")
    ...
    override fun onCleared() {
        ...
        releaseBackgroundResources()
        voice.shutdown()
        io.github.saltmango.rere.data.RideDiagnostics.stop("app closed")
    }
}
