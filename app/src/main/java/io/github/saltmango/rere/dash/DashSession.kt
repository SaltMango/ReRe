package io.github.saltmango.rere.dash

import android.util.Log
import io.github.saltmango.rere.dash.protocol.DashCommands
import io.github.saltmango.rere.util.Dbg
import io.github.saltmango.rere.dash.protocol.K1GPacket
...
class DashSession(private val scope: CoroutineScope) {
    companion object {
        ...
        private const val HOSTNAME       = "reRe"
    }
    ...
    @Volatile var destinationName: String = "reRe"
    ...
    fun updateRouteCard(name: String) {
        destinationName = name.ifBlank { "reRe" }
    ...
    private suspend fun runSession(ssid: String, network: android.net.Network? = null) {
        try {
            ...
            io.github.saltmango.rere.data.RideDiagnostics.log("auth", "initial burst sent — waiting up to ${AUTH_TIMEOUT}ms for 07 01 01")
            ...
            io.github.saltmango.rere.data.RideDiagnostics.log("auth", "authenticated (07 01 01) — entering nav mode")

            enterNavMode(sock)
            _state.value = DashState.READY
            Log.i(TAG, "READY ✓")
            io.github.saltmango.rere.data.RideDiagnostics.log("session", "nav-mode kick sent — READY (waiting for video pipeline)")
        ...
        rxJob = scope.launch(Dispatchers.IO) {
            while (isActive) {
                val pkt = try {
                    ...
                    io.github.saltmango.rere.data.RideDiagnostics.log("error", "RX loop stopped — socket error: ${e.message}")
    ...
    private fun dispatchIncoming(pkt: ByteArray, sock: DashSocket) {
        ...
        for (tlv in tlvs) {
            ...
            if (tlv.type == 0x07) {
                when (val ev = auth?.ingest(tlv)) {
                    is AuthEvent.SendKey -> {
                        ...
                        io.github.saltmango.rere.data.RideDiagnostics.log("auth", "RSA pubkey received → sending session key (q3c.d)")
                        sock.send(ev.packet)
                    }
                    ...
                    AuthEvent.Rejected -> {
                        ...
                        io.github.saltmango.rere.data.RideDiagnostics.log("auth", "REJECTED — retry #$authRetries")
                        auth?.reset()
            ...
            if (tlv.type == 0x09 && tlv.sub == 0x06 &&
                ...
                if (!loggedFirstAck) {
                    loggedFirstAck = true
                    io.github.saltmango.rere.data.RideDiagnostics.log("dash", "dash DECODED first IDR (09 06 55) — video accepted ✓")
                }
        ...
        heartbeatJob = scope.launch(Dispatchers.IO) {
            ...
            while (isActive) {
                ...
                if (_state.value == DashState.STREAMING && loggedFirstAck &&
                    ...
                    io.github.saltmango.rere.data.RideDiagnostics.log("error", "RX watchdog: dash silent ${silent}s → link lost")
            ...
            while (isActive && _state.value == DashState.STREAMING) {
                ...
                when {
                    c != null -> {
                        ...
                        if (!loggedCall) { loggedCall = true
                            io.github.saltmango.rere.data.RideDiagnostics.log("media", "TX call card 05 22: $c") }
                    }
                ...
                npTitle?.let {
                    ...
                    if (!loggedNp) { loggedNp = true
                        io.github.saltmango.rere.data.RideDiagnostics.log("media", "TX now-playing 05 0d: $it") }
                }
                delay(ROUTE_CARD_MS)
            }
        }
    ...
    private fun fail(msg: String) {
        Log.e(TAG, "ERROR — $msg")
        io.github.saltmango.rere.data.RideDiagnostics.log("error", "session fail: $msg")
