...
class GarageViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private fun reload() = viewModelScope.launch {
        ...
        withContext(Dispatchers.IO) {
            io.github.saltmango.rere.data.MaintenanceNotifier.check(getApplication(), ui.maint.map { it.item }, ui.odometerKm.toInt())
            io.github.saltmango.rere.data.MaintenanceNotifier.checkDocuments(getApplication(), ui.documents)
        }
    }
    ...
    fun saveDocument(
        ...
        withContext(Dispatchers.IO) {
            ...
            repo.upsertDocument(VehicleDocument(
                sid = sid ?: ReReDb.newSid(), type = type, title = title, number = number,
