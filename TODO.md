...
**Navigation → Power efficiency → Maps↔reRe↔Dash integration → correctness →
...
- [x] **In-app Dash view = real Google Maps.** Replaced the decorative `CircularDash`
  (which drew a fake random polyline) with `reReMap` (maps-compose `GoogleMap`)
...
- [ ] **ADD YOUR MAPS API KEY** to `local.properties` → `MAPS_API_KEY=AIza…`.
  ...
  create an API key, restrict to package `io.github.saltmango.rere` + SHA-1
...
## P1 — GOOGLE MAPS ↔ RERE ↔ DASH — IMPLEMENTED, needs on-device validation
...
- [x] **Local persistence (SQLite).** `data/reReDb` (hand-rolled
...
- [~] **MapLibre + OSM migration.** Decided: **OpenFreeMap** (keyless vector, look-first;
  ...
  - [x] **In-app map → MapLibre + OpenFreeMap.** `reReMap` rewritten on `MapView`
...
- [x] **Stable release signing.** Promoted the original debug keystore (the key the first
  ...
  at `~/keystores/rere-release.jks`. Wired via gitignored `signing.properties` →
...
- [ ] **Release process (per update):** bump version → `./gradlew :app:assembleRelease` →
  `gh release create vX.Y dist/rere-X.Y.apk --notes-file dist/RELEASE-X.Y.md`.
- [x] **In-app updater ("OTA feel").** `data/UpdateChecker` GETs
  `api.github.com/repos/adityadasika21/reRe/releases/latest` on app open (background),
...
- [x] **App icon** — reRe compass star (`~/Downloads/reRe.png`) wired as
...
- [ ] **Kill the duplicate app entries + work-profile install.** Likely an old
  `com.example.rere` build (default id, pre-`io.github.saltmango.rere`) still on the
  device → two icons. When the phone's back: `adb uninstall com.example.rere`,
  `adb uninstall io.github.saltmango.rere`, then reinstall to the personal user only:
